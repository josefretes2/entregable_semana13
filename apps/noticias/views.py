...
class AgregarLibro(CreateView, LoginRequiredMixin):
    ...
    template_name = 'Noticias/agregar_libro.html'
    success_url = reverse_lazy('inicio')
...
class ListarLibros(ListView):
    model = Noticias
    template_name = 'Noticias/listar_libros.html'
...
def ListarLibrosPorCategoria(request,categoria):
    ...
    libros = Noticias.objects.filter(categoria = categorias2[0].id).order_by('fecha_agregado')
    categorias = Categorias.objects.all()
    template_name='Noticias/listar_libros.html'
    contexto = {
        'libros': libros,
...
def libro_detalle(request,id):
    libros = Noticias.objects.get(id=id)
...
def ordenar_libros_por(request):
    ...
    if orden == 'fecha':
        libros = Noticias.objects.order_by('fecha_agregado')  # Ordenar por fecha(Si quisiera desc seria: ('-fecha_agregado'))
    elif orden == 'titulo':
        libros = Noticias.objects.order_by('titulo')  # Ordenar por título(Si quisiera desc seria: ('-titulo'))
    else:#Si no hay nada solo todos sin orden 
        libros = Noticias.objects.all()  # Obtener todos los artículos sin ordenar
